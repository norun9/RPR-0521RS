...
#define ALS_DATA0_LSB_REG 0x46 // ALSのデータレジスタ（下位バイト）
#define RPR0521RS_ADDR 0x38
#define ACK_CHECK_EN 0x1

static esp_err_t read_from_rpr0521rs(uint16_t *als_value)
{
    uint8_t data_rd[2] = {0}; // データを格納するためのバッファ
    ...
    i2c_master_start(cmd); // リピートスタートコンディション
    ...
    i2c_master_stop(cmd); // ストップコンディションを送信
...
esp_err_t setup_sensor_config(i2c_port_t i2c_num, uint8_t reg_addr, uint8_t value)
{
    esp_err_t ret;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
...
esp_err_t rpr0521_init(void)
{
    esp_err_t ret;
    // setup MODE_CONTROL
    ret = setup_sensor_config(I2C_MASTER_NUM, 0x41, 0x8A);
    if (ret != ESP_OK)
    {
...
void app_main(void)
{
    ...
    uint16_t als_value;
    ESP_ERROR_CHECK(read_from_rpr0521rs(&als_value));
    printf("data:%d\n", als_value);
